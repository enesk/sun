 ...
 * gesetzt und eine BudgetExceededException geworfen. Die Pause endet um 00:00
...
class BudgetGuard
{
    ...
    /**
     * @param  string|null  $referenceType  z.B. 'article_drafts'
     *
    ...
    public function check(
        ...
        $this->assertBelow(
            BudgetExceededException::SCOPE_DAILY,
            $provider,
            $spentTotal,
            $dailyLimit,
            $state,
        ...
        $this->assertBelow(
            BudgetExceededException::SCOPE_PROVIDER,
            $provider,
        ...
        if ($referenceType !== null && $referenceId !== null) {
            $this->assertBelow(
                BudgetExceededException::SCOPE_ARTICLE,
                $provider,
    ...
    /**
     * Schreibt den Tageszaehler des Providers fort. Wird vom LlmClient und vom
     * EmbeddingClient nach jedem Aufruf gerufen, auch nach einem gescheiterten
     ...
     */
    public function pause(ProviderState $state, BudgetExceededException $exception): void
    {
        if ($state->isPaused()) {
            return;
        }
    ...
    private function assertBelow(
        ...
        $exception = BudgetExceededException::forScope($scope, $provider, $spent, $limit, $tenantId);
