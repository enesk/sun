<div>
    {{-- Status Tabs --}}
    <div class="flex items-center gap-1 mb-4 border-b border-base-200 overflow-x-auto">
        @php
            $tabs = [
        ...
        @foreach($tabs as $value => $tab)
            <button wire:click="$set('filterStatus', '{{ $value }}')"
                    class="px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors
                           {{ $filterStatus === $value
                               ? 'border-current text-[var(--portal-primary,#3b82f6)]'
                               : 'border-transparent text-base-content/50 hover:text-base-content/70 hover:border-base-300' }}">
                {{ $tab['label'] }}
                @if($tab['count'] > 0)
                    <span class="ml-1.5 inline-flex items-center justify-center px-1.5 py-0.5 rounded-full text-xs font-medium
                                 {{ $value === 'pending' && $tab['count'] > 0 ? 'bg-amber-100 text-amber-700' : 'bg-base-200 text-base-content/50' }}">
                        {{ $tab['count'] }}
                    </span>
                @endif
            </button>
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text"
                           ...
                           style="--tw-ring-color: var(--portal-primary, #3b82f6);">
                </div>
            </div>
        ...
        @if(count($selected) > 0)
            <div class="mt-3 pt-3 border-t border-base-200 flex items-center gap-3">
                ...
                <button wire:click="bulkApprove"
                        class="text-xs px-3 py-1.5 rounded-lg bg-green-100 text-green-700 hover:bg-green-200 transition-colors font-medium">
                    Freigeben
                </button>
    ...
    <div class="space-y-3">
        @forelse($reviews as $review)
            <div class="card-portal" wire:key="review-{{ $review->id }}">
                <div class="flex items-start gap-3">
                    {{-- Checkbox --}}
                    <input type="checkbox"
                           wire:model.live="selected"
                           value="{{ $review->id }}"
                           class="mt-1 rounded border-base-300 text-blue-600 focus:ring-blue-500 shrink-0"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-2 mb-1.5">
                            ...
                            <div class="flex items-center gap-0.5" aria-label="{{ number_format($review->rating, 1) }} von 5 Sternen">
                                @for($i = 1; $i <= 5; $i++)
                                    <svg class="w-4 h-4 {{ $i <= $review->rating ? 'text-yellow-400' : 'text-base-200' }}" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                @endfor
                                <span class="text-xs text-base-content/40 ml-1">{{ number_format($review->rating, 1) }}</span>
                            </div>
                        ...
                        {{-- Title --}}
                        @if($review->title)
                            <h3 class="text-sm font-semibold text-base-content mb-1">{{ $review->title }}</h3>
                        @endif

                        {{-- Body --}}
                        @if($review->body)
                            <p class="text-sm text-base-content/70 mb-2 line-clamp-3">{{ $review->body }}</p>
                        @endif
                        ...
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-base-content/50">
                            ...
                            @if($review->moderation_note)
                                <span class="inline-flex items-center gap-1 text-amber-600" title="{{ $review->moderation_note }}">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        ...
                        @if($isAdmin)
                            <button wire:click="deleteReview({{ $review->id }})"
                                    ...
                                    class="p-1.5 rounded-lg text-base-content/40 hover:text-red-600 hover:bg-red-50 transition-colors"
                                    title="Löschen">
        ...
        @empty
            <div class="card-portal text-center py-12">
                ...
                @if($search || $filterStatus || $filterCompany || $filterRating)
                    <p class="text-sm text-base-content/50 mb-2">Keine Bewertungen gefunden</p>
                    <button wire:click="resetFilters" class="text-sm font-medium hover:underline" style="color: var(--portal-primary, #3b82f6);">
                        Filter zurücksetzen
                    </button>
                @else
                    <p class="text-sm text-base-content/50">Noch keine Bewertungen vorhanden</p>
                @endif
            </div>
        @endforelse
    </div>
    ...
    {{-- ================================================================ --}}
    @if($showRejectModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center px-4" role="dialog" aria-modal="true" aria-labelledby="reject-modal-title">
            {{-- Overlay --}}
            <div class="absolute inset-0 bg-black/40" wire:click="$set('showRejectModal', false)"></div>

            {{-- Modal --}}
            <div class="relative bg-base-100 rounded-xl shadow-xl max-w-sm w-full p-6 z-10">
                <h3 id="reject-modal-title" class="text-lg font-semibold text-base-content mb-2">
                ...
                <textarea wire:model="rejectReason"
                          ...
                          class="w-full px-3 py-2.5 text-sm border border-base-200 rounded-lg bg-base-100 text-base-content placeholder-base-content/40 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent resize-y mb-4"></textarea>
