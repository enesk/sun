    ...
    <form wire:submit="save">
        ...
        <div x-show="tab === 'seo'" x-cloak>
            <div class="space-y-6">
                ...
                <div class="dash-card dash-card-padded space-y-4">
                    <div>
                        <h3 class="dash-form-section-title">Sitemap & Robots.txt</h3>
                        <p class="dash-input-hint">XML-Sitemap für Google Search Console generieren</p>
                    </div>

                    @if($sitemapInfo['exists'])
                        <div class="flex items-start gap-3 p-3 rounded-lg" style="background: rgba(22, 163, 74, 0.06); border: 1px solid rgba(22, 163, 74, 0.15);">
                            <svg class="w-5 h-5 mt-0.5 shrink-0 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            <div class="text-sm">
                                <p class="font-semibold text-green-700">Sitemap vorhanden</p>
                                <p class="text-xs mt-1" style="color: var(--dash-text-muted);">
                                    Zuletzt generiert: {{ $sitemapInfo['lastGenerated'] }} · {{ $sitemapInfo['size'] }} KB
                                    @if($sitemapInfo['robotsExists'])
                                        · robots.txt vorhanden
                                    @endif
                                </p>
                                @if($sitemapInfo['url'])
                                    <a href="{{ $sitemapInfo['url'] }}" target="_blank" rel="noopener"
                                       class="inline-flex items-center gap-1 text-xs font-medium mt-2 hover:underline" style="color: var(--portal-primary, #3b82f6);">
                    ...
                    @else
                        <div class="flex items-start gap-3 p-3 rounded-lg" style="background: rgba(245, 158, 11, 0.06); border: 1px solid rgba(245, 158, 11, 0.15);">
                            <svg class="w-5 h-5 mt-0.5 shrink-0 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
                            <div class="text-sm">
                                <p class="font-semibold text-amber-700">Keine Sitemap vorhanden</p>
                                <p class="text-xs mt-1" style="color: var(--dash-text-muted);">Klicken Sie auf "Sitemap generieren" um eine XML-Sitemap zu erstellen.</p>
                            </div>
                        </div>
                    @endif
                    ...
                    <p class="dash-input-hint">
                        ...
                        Laden Sie die Sitemap-URL anschließend in der
                        <a href="https://search.google.com/search-console" target="_blank" rel="noopener" class="hover:underline" style="color: var(--portal-primary, #3b82f6);">Google Search Console</a>
                        hoch.
                    </p>
                </div>
