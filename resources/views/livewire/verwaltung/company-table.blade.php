        ...
        @if(count($selected) > 0)
            <div class="mt-3 pt-3 border-t border-base-200 flex items-center gap-3">
                ...
                <button wire:click="bulkToggleActive(false)"
                        class="text-xs px-3 py-1.5 rounded-lg bg-base-200 text-base-content/70 hover:bg-base-300 transition-colors font-medium">
                    Deaktivieren
                </button>
            </div>
        @endif
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-base-200 text-left">
                        ...
                        <th class="p-3 hidden lg:table-cell">Kategorien</th>
                        <th class="p-3 hidden md:table-cell text-center">
                            <button wire:click="sort('rating')" class="flex items-center gap-1 font-semibold text-base-content/70 hover:text-base-content">
                                Bewertung
                                @if($sortBy === 'rating')
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $sortDir === 'asc' ? 'M4.5 15.75l7.5-7.5 7.5 7.5' : 'M19.5 8.25l-7.5 7.5-7.5-7.5' }}"/></svg>
                                @endif
                        ...
                        @if($isAdmin)
                            <th class="p-3 text-center hidden lg:table-cell">Premium</th>
                        @endif
                ...
                <tbody>
                    @forelse($companies as $company)
                        <tr class="border-b border-base-200/50 hover:bg-base-200/30 transition-colors" wire:key="company-{{ $company->id }}">
                            {{-- Checkbox --}}
                            <td class="p-3">
                                <input type="checkbox"
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <a href="{{ route('verwaltung.companies.edit', $company->id) }}"
                                           class="text-sm font-medium text-base-content hover:underline block truncate">
                                            {{ $company->name }}
                                        </a>
                                        @if($isAdmin && $company->owner)
                                            <span class="text-xs text-base-content/40">{{ $company->owner->name }}</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="p-3 hidden lg:table-cell">
                                <div class="flex flex-wrap gap-1">
                                    ...
                                    @if($company->categories->count() > 2)
                                        <span class="text-xs text-base-content/40">+{{ $company->categories->count() - 2 }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="p-3 hidden md:table-cell text-center">
                                @if($company->rating_count > 0)
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <span class="text-xs text-base-content/40">({{ $company->rating_count }})</span>
                                    </div>
                                @else
                            ...
                            <td class="p-3 text-center">
                                <button wire:click="toggleActive({{ $company->id }})"
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium transition-colors cursor-pointer
                                            {{ $company->is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-base-200 text-base-content/50 hover:bg-base-300' }}"
                            ...
                            @if($isAdmin)
                                <td class="p-3 text-center hidden lg:table-cell">
                                    <button wire:click="togglePremium({{ $company->id }})"
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium transition-colors cursor-pointer
                                                {{ $company->is_premium ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-base-200 text-base-content/30 hover:bg-base-300' }}"
                        ...
                        <tr>
                            <td colspan="{{ $isAdmin ? 8 : 7 }}" class="p-12 text-center">
                                ...
                                @if($search || $filterCity || $filterCategory || $filterStatus)
                                    <p class="text-sm text-base-content/50 mb-2">Keine Firmen gefunden</p>
                                    <button wire:click="resetFilters" class="text-sm font-medium hover:underline" style="color: var(--portal-primary, #3b82f6);">
                                        Filter zurücksetzen
                                    </button>
                                @else
                                    <p class="text-sm text-base-content/50 mb-2">Noch keine Firmen eingetragen</p>
                                    <a href="{{ route('verwaltung.companies.create') }}" class="text-sm font-medium hover:underline" style="color: var(--portal-primary, #3b82f6);">
