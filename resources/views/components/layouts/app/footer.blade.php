@php
    $tenant = tenant();
    $hasTenantBranding = !empty($tenant);
    $brandingService = $hasTenantBranding ? app(\App\Services\TenantBrandingService::class) : null;
    $tenantLogo = $hasTenantBranding ? $tenant->getAttribute('branding.logo_path') : null;
    $tenantSocials = [];
    if ($hasTenantBranding) {
        if ($tenant->getAttribute('settings.social_facebook')) $tenantSocials['facebook'] = $tenant->getAttribute('settings.social_facebook');
        if ($tenant->getAttribute('settings.social_instagram')) $tenantSocials['instagram'] = $tenant->getAttribute('settings.social_instagram');
        if ($tenant->getAttribute('settings.social_twitter')) $tenantSocials['twitter'] = $tenant->getAttribute('settings.social_twitter');
        if ($tenant->getAttribute('settings.social_linkedin')) $tenantSocials['linkedin'] = $tenant->getAttribute('settings.social_linkedin');
    }
@endphp
<footer class="bg-primary-500 text-white mt-12">
    <div class="mx-auto w-full max-w-(--breakpoint-xl) p-4 py-6 lg:py-8">
        <div class="md:flex md:justify-between">
            <div class="mb-6 md:mb-0">
                <a href="/" class="flex items-center">
                    @if($tenantLogo)
                        <img src="{{ asset($tenantLogo) }}" class="h-6 me-3" alt="{{ $tenant->name ?? config('app.name') }}" />
                    @else
        ...
        <div class="sm:flex sm:items-center sm:justify-between">
          <span class="text-xs text-primary-100 sm:text-center dark:text-gray-400">
            @if($hasTenantBranding && $brandingService)
                {{ $brandingService->getFooterText($tenant) }}
            @else
                © {{ date('Y') }} <a href="/" class="hover:underline text-primary-100">{{ config('app.name') }}™</a>. {{ __('All rights reserved.') }}
            @endif
          </span>
            <div class="flex gap-3 mt-4 sm:justify-center sm:mt-0">
                @if(count($tenantSocials) > 0)
                    {{-- Tenant-spezifische Social Links --}}
                    @foreach($tenantSocials as $platform => $url)
                        <x-link.social-icon name="{{ $platform === 'twitter' ? 'x' : $platform }}" title="{{ ucfirst($platform) }}" link="{{ $url }}" class="text-primary-100 border-primary-200 hover:text-primary-50"/>
                    @endforeach
                @else
                    {{-- Fallback: App-Config Social Links --}}
