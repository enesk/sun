<div>
    ...
    @if($showForm && !$submitted)
        <div id="review-form-{{ $company->id }}"
             class="mt-4 company-review-card !p-4 sm:!p-5"
             x-data="{
                 hoverRating: 0,
            ...
            <h3 class="text-sm sm:text-base font-semibold text-[#0F172A] mb-3 sm:mb-4">
            ...
            <div class="mb-4 sm:mb-5">
                ...
                <div class="flex items-center gap-1 sm:gap-2" role="radiogroup" aria-label="Bewertung in Sternen">
                    @for($i = 1; $i <= 5; $i++)
                        {{-- Ein Stern = ein Container mit zwei unsichtbaren Klick-Zonen --}}
                        <div class="relative w-11 h-11 sm:w-10 sm:h-10 cursor-pointer" @mouseleave="hoverRating = 0">
                            {{-- Voller Stern SVG (Hintergrund) --}}
                            <svg class="w-11 h-11 sm:w-10 sm:h-10 transition-colors duration-150 pointer-events-none"
                            ...
                            <svg class="w-11 h-11 sm:w-10 sm:h-10 absolute inset-0 transition-colors duration-150 pointer-events-none"
                                 :class="(hoverRating || selectedRating) >= {{ $i - 0.5 }} ? 'text-portal-accent' : 'text-gray-300'"
                                 fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"
                                 style="clip-path: inset(0 50% 0 0);">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                            {{-- Linke Klick-Zone (halber Stern) — Touch-Padding für 44px Minimum --}}
                            <button type="button"
                                    wire:click="setRating({{ $i - 0.5 }})"
                                    @mouseenter="hoverRating = {{ $i - 0.5 }}"
                                    class="absolute inset-y-0 left-0 w-1/2 min-w-[22px] focus:outline-none focus-visible:ring-2 focus-visible:ring-portal-primary rounded-l"
                                    role="radio"
                            ...
                            <button type="button"
                                    ...
                                    class="absolute inset-y-0 right-0 w-1/2 min-w-[22px] focus:outline-none focus-visible:ring-2 focus-visible:ring-portal-primary rounded-r"
                                    role="radio"
                    ...
                    <span class="ml-2 sm:ml-3 text-sm font-medium text-[#64748B]"
            ...
            {{-- Name + Titel auf Mobile gestackt, auf Desktop nebeneinander --}}
            ...
            <div class="mb-4 sm:mb-5">
                ...
                <textarea
                    ...
                    rows="3"
                    maxlength="2000"
                    placeholder="Beschreiben Sie Ihre Erfahrung..."></textarea>
            ...
            {{-- Hinweis --}}
            <p class="text-xs text-[#94A3B8] mb-3 sm:mb-4">
            ...
            <div class="flex flex-col-reverse sm:flex-row sm:items-center gap-2 sm:gap-3">
                <button
                    ...
                    class="btn-portal inline-flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-3 sm:py-2.5 rounded-lg text-sm font-medium ripple"
                ...
                <button
                    wire:click="toggleForm"
                    class="text-sm text-[#64748B] hover:text-[#0F172A] transition-colors px-3 py-2.5 text-center sm:text-left">
                    Abbrechen
                </button>
            </div>
        </div>
    ...
    {{-- Erfolgs-Meldung --}}
    @if($submitted)
        <div class="mt-4 company-review-card !p-4 sm:!p-5 border-green-200 bg-green-50/50">
            <div class="flex items-start gap-3">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-green-100 flex items-center justify-center shrink-0">
