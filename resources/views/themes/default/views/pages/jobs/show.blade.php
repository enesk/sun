...
@section('content')
    ...
    {!! json_encode(array_filter([
        ...
        'hiringOrganization' => $job->company ? [
            ...
            'logo' => $job->company->getFirstMediaUrl('logo') ?: null,
        ] : null,
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-start gap-4 mb-4">
                        ...
                        @if($job->company)
                            <div class="w-16 h-16 rounded-xl bg-base-200 overflow-hidden shrink-0 ring-1 ring-base-200">
                                @if($job->company->getFirstMediaUrl('logo', 'thumb'))
                                    <img src="{{ $job->company->getFirstMediaUrl('logo', 'thumb') }}" alt="{{ $job->company->name }}" class="w-full h-full object-cover">
                                @else
                    ...
                    <section class="mb-8 job-company-box" aria-labelledby="job-company-title">
                        ...
                        <div class="flex items-start gap-4">
                            <div class="w-12 h-12 rounded-lg bg-base-200 overflow-hidden shrink-0">
                                @if($job->company->getFirstMediaUrl('logo', 'thumb'))
                                    <img src="{{ $job->company->getFirstMediaUrl('logo', 'thumb') }}" alt="" class="w-full h-full object-cover">
                                @else
                            ...
                            <div class="space-y-2.5 text-sm">
                                ...
                                @if($job->company->tel)
                                    <div class="flex items-center gap-2">
                                        ...
                                        <x-phone-link :number="$job->company->tel" class="text-portal-primary hover:underline">{{ $job->company->tel }}</x-phone-link>
                                    </div>
                                @endif
