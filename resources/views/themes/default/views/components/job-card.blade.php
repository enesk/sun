...
@php
    $layout = $layout ?? 'list';
    $company = $job->company;
    // Top-Job: Feature job_highlight ueber den Entitlement-Service (#13)
    $isPremium = $company && $job->is_top_job;
@endphp
...
@else
    ...
    <article class="job-card {{ $isPremium ? 'job-card--premium' : '' }} group"
             aria-label="Stellenanzeige: {{ $job->title }}{{ $isPremium ? ' ('.__('portal.jobs.card.top_job').')' : '' }}">
