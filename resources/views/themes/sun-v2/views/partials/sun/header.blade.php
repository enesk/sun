@php
    ...
    $navLinks = [
        ['label' => 'Städte', 'href' => route('portal.cities.index')],
        ['label' => 'Ratgeber', 'href' => route('portal.blog.index')],
        ['label' => 'Stellenanzeigen', 'href' => route('portal.jobs.index')],
    ];

    $cta = $ownedCompany
        ? ['label' => 'Mein Firmenprofil', 'href' => route('portal.owner.dashboard')]
        : ['label' => 'Firma eintragen', 'href' => route('portal.companies.create')];
...
<header class="sticky top-0 z-40 bg-white border-b border-zinc-200">
  <div class="container-portal flex items-center justify-between h-16">
    <a href="{{ route('home') }}" class="flex items-center gap-2" aria-label="{{ $portalName }} – Startseite">
    ...
    <nav class="hidden xl:flex items-center gap-1" aria-label="Hauptnavigation">
      ...
      @auth
        <form method="POST" action="{{ route('logout') }}">
          @csrf
          <button type="submit" class="btn-ghost px-3">Abmelden</button>
        </form>
      @else
        <a href="{{ route('login') }}" class="btn-ghost px-3">Anmelden</a>
      @endauth
      @if($cta)<a href="{{ $cta['href'] }}" class="btn-primary ml-1">{{ $cta['label'] }}</a>@endif
    </nav>
    <button type="button" data-menu-toggle class="xl:hidden btn-ghost px-3" aria-label="Menü öffnen" aria-expanded="false" aria-controls="mobileMenu">
  ...
  <div id="mobileMenu" class="hidden xl:hidden border-t border-zinc-200 bg-white">
    <nav class="container-portal py-3 flex flex-col gap-1" aria-label="Mobile Navigation">
      @foreach($navLinks as $link)
      ...
      @auth
        <form method="POST" action="{{ route('logout') }}" class="flex flex-col">
          @csrf
          <button type="submit" class="btn-ghost justify-start">Abmelden</button>
        </form>
      @else
        <a href="{{ route('login') }}" class="btn-ghost justify-start">Anmelden</a>
      @endauth
