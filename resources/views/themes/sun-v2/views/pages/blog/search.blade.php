...
@section('title', ($searchTerm ? __('portal.blog.search.meta_title', ['begriff' => $searchTerm]).' — ' : '').__('portal.layout.header.guide').' — '.$portalName)
  ...
  <div class="container-portal">
    ...
    <div class="mt-4 max-w-2xl">
      <h1 class="text-3xl md:text-4xl font-bold tracking-tight text-zinc-900">
        @if($searchTerm)
          {{ __('portal.blog.search.heading', ['begriff' => $searchTerm]) }}
        @else
          {{ __('portal.blog.list.search_label') }}
        @endif
      </h1>
      <p class="mt-3 text-base md:text-lg leading-relaxed text-zinc-700">{{ trans_choice('portal.blog.search.result_count', $total, ['anzahl' => number_format($total, 0, ',', '.')]) }}</p>
    </div>
