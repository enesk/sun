...
@php
    ...
    $total = $posts->total();
    $intro = "Alle Ratgeber-Artikel mit dem Schlagwort {$tag->name}.";
    $otherTags = $popularTags->where('id', '!=', $tag->id)->take(12);
@endphp

@section('title', '#'.$tag->name.' — Ratgeber — '.$portalName)
...
<section class="bg-brand-50 py-10 md:py-16">
  <div class="container-portal">
    ...
    <div class="mt-4 max-w-2xl">
      <span class="pill-brand">Schlagwort</span>
      ...
      <p class="mt-2 text-sm text-zinc-500">{{ number_format($total, 0, ',', '.') }} Artikel · <a href="{{ route('portal.blog.editorial') }}" class="hover:text-brand underline">So arbeitet unsere Redaktion</a></p>
    </div>
...
<div class="container-portal py-12 md:py-16 flex flex-col gap-12 md:gap-16">
  ...
  <section>
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 sm:gap-4">
      <h2 class="text-2xl font-semibold text-zinc-900">Artikel zu #{{ $tag->name }}</h2>
      @if($otherTags->isNotEmpty())
        <nav class="flex gap-2 -mx-4 px-4 sm:mx-0 sm:px-0 overflow-x-auto scroll-snap pb-1 min-w-0" aria-label="Weitere Schlagwörter">
    ...
    @if($posts->isEmpty())
      <div class="card p-8 mt-6 text-center">
        ...
        <p class="mt-4 font-semibold text-zinc-900">Noch keine Artikel zu #{{ $tag->name }}</p>
