...
@php
    ...
    if (! empty($faq)) {
        $mainHeadings[] = ['id' => 'faq', 'text' => 'Häufige Fragen', 'level' => 2];
    }
...
<div class="container-portal pt-6 pb-12 md:pb-16">
  <div class="grid gap-8 xl:grid-cols-[minmax(0,1fr)_18rem] xl:items-start">
    <article class="min-w-0 max-w-3xl">
      ...
      <div class="mt-4 flex flex-wrap items-center gap-3">
        ...
        @if($post->reading_time_minutes)
          <span class="text-sm text-zinc-500">{{ $post->reading_time_minutes }} Min. Lesezeit</span>
        @endif
        @if($seo['modified_at'])
          <span class="text-sm text-zinc-500">Aktualisiert am <time datetime="{{ $seo['modified_at']->toDateString() }}">{{ $seo['modified_at']->translatedFormat('j. F Y') }}</time></span>
        @endif
        {{-- KI-Transparenz (#27) --}}
        <a href="{{ route('portal.blog.editorial') }}" class="text-sm text-zinc-500 hover:text-brand underline">Maschinell erstellt, redaktionell geprüft</a>
      </div>
      ...
      <div class="mt-8 {{ $proseClasses }}">
        ...
        @if(!empty($region))
          <section class="rounded-2xl bg-brand-50 p-5 flex gap-4" aria-labelledby="region-heading">
            ...
            <div class="min-w-0">
              <h2 id="region-heading" class="!text-lg !pt-0">Was in {{ $region['name'] }} gilt</h2>
              @if($region['intro'])
              ...
              @endif
              <a href="{{ $region['url'] }}" class="btn-secondary mt-4 !no-underline !text-zinc-900">Anbieter in {{ $region['name'] }} vergleichen</a>
            </div>
          </section>
        @endif
      ...
      <div class="card p-5 md:p-8 mt-10 bg-brand-50 border-brand-100">
        <h2 class="text-2xl font-semibold text-zinc-900">{{ $branchSingular }} aus {{ !empty($region) ? $region['name'] : 'deiner Nähe' }}</h2>
        <p class="mt-2 text-zinc-700 leading-relaxed">
          @if($ctaCount > 0)
            {{ number_format($ctaCount, 0, ',', '.') }} Betriebe im Portal. Gib deinen Ort ein und vergleiche zwei bis drei Angebote.
          @else
            Gib deinen Ort ein und vergleiche zwei bis drei Angebote.
          @endif
        </p>
        <form action="{{ route('portal.companies.index') }}" method="get" role="search" class="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
          ...
          <input id="cta-ort" name="ort" class="input" placeholder="Ort oder PLZ" value="{{ $region['name'] ?? '' }}" autocomplete="postal-code">
          <button type="submit" class="btn-primary">{{ $branchSingular }} finden</button>
        </form>
      </div>

      <!-- Quellen und Aktualitaet (#27) -->
      <section class="mt-8 text-sm text-zinc-500" @if(!empty($sources)) aria-labelledby="sources-heading" @else aria-label="Aktualität dieses Beitrags" @endif>
        <p>
          @if($seo['published_at'])
            Erstellt am <time datetime="{{ $seo['published_at']->toDateString() }}">{{ $seo['published_at']->translatedFormat('j. F Y') }}</time>
          @endif
          @if($seo['published_at'] && $seo['modified_at'])
            <span aria-hidden="true">&middot;</span>
          @endif
          @if($seo['modified_at'])
            Zuletzt geprüft am <time datetime="{{ $seo['modified_at']->toDateString() }}">{{ $seo['modified_at']->translatedFormat('j. F Y') }}</time>
          @endif
        </p>
        @if(!empty($sources))
          <h2 id="sources-heading" class="mt-4 font-semibold text-zinc-900">Verwendete Quellen</h2>
      ...
      @if(!empty($changelog ?? []))
        <section class="mt-6 text-sm" aria-labelledby="changelog-heading">
          <h2 id="changelog-heading" class="font-semibold text-zinc-900">Aktualisierungen</h2>
          <ol class="mt-2 divide-y divide-zinc-100">
            @foreach($changelog as $entry)
              <li class="py-2">
                <time class="text-zinc-500" datetime="{{ $entry['at']->toDateString() }}">Aktualisiert am {{ $entry['at']->translatedFormat('j. F Y') }}</time>
                <p class="mt-0.5">{{ $entry['summary'] }}</p>
                @if(!empty($entry['reasons']))
                  <p class="text-zinc-500">Anlass: {{ implode(', ', $entry['reasons']) }}</p>
                @endif
              </li>
            @endforeach
          </ol>
      ...
      @if($relatedPosts->isNotEmpty())
        <section class="mt-12">
          <h2 class="text-2xl font-semibold text-zinc-900">Passende Artikel</h2>
            ...
            @foreach($relatedPosts->take(2) as $related)
              <a href="{{ route('portal.blog.show', $related->slug) }}" class="card-interactive p-5 flex flex-col gap-2">
                ...
                @if($related->reading_time_minutes)
                  <span class="text-sm text-zinc-500 mt-auto pt-1">{{ $related->reading_time_minutes }} Min. Lesezeit</span>
                @endif
              </a>
            @endforeach
          </div>
    ...
    <!-- Rechte Spalte -->
    <aside class="hidden xl:block" aria-label="Weitere Informationen zum Artikel">
      <div class="sticky top-24 flex flex-col gap-4">
        @if(count($mainHeadings) >= 3)
          <nav class="card p-5" aria-label="Inhalt (Seitenleiste)">
