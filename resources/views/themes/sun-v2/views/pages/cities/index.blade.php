  ...
  <div class="container-portal">
    ...
    <form action="{{ route('portal.cities.index') }}" method="get" role="search" class="card shadow-lg p-4 md:p-5 mt-6 md:mt-8 max-w-2xl">
      ...
      <div class="grid gap-3 grid-cols-[1fr_auto]">
        <div class="relative">
          <label class="sr-only" for="suche">{{ __('portal.cities.search.label') }}</label>
          <x-sun.icon name="map-pin" class="icon absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" />
          <input id="suche" name="suche" type="search" class="input pl-10" value="{{ $citiesSun['term'] }}" placeholder="{{ $citiesSun['land'] ? __('portal.cities.search.placeholder_land', ['land' => $citiesSun['land']]) : __('portal.cities.search.placeholder') }}" autocomplete="address-level2">
        </div>
        <button type="submit" class="btn-primary px-4 sm:px-5">{{ __('portal.cities.search.submit') }}</button>
      </div>
    </form>

    @if($citiesSun['states'] !== [])
      <div class="mt-4 -mx-4 px-4 sm:mx-0 sm:px-0 flex items-center gap-2 overflow-x-auto scroll-snap pb-1" role="group" aria-label="{{ __('portal.cities.states.label') }}">
        <span class="text-sm text-zinc-500 mr-1 whitespace-nowrap">{{ __('portal.cities.states.prefix') }}</span>
    ...
    <dl class="mt-8 md:mt-10 grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4 max-w-2xl">
      <div><dt class="text-sm text-zinc-500">{{ __('portal.cities.stats.businesses') }}</dt><dd class="text-xl font-semibold text-zinc-900">{{ number_format($citiesSun['totalCompanies'], 0, ',', '.') }}</dd></div>
      <div><dt class="text-sm text-zinc-500">{{ $citiesSun['land'] ? __('portal.cities.stats.places') : __('portal.layout.header.cities') }}</dt><dd class="text-xl font-semibold text-zinc-900">{{ number_format($citiesSun['totalCities'], 0, ',', '.') }}</dd></div>
      @unless($citiesSun['land'])
        <div><dt class="text-sm text-zinc-500">{{ __('portal.cities.stats.states') }}</dt><dd class="text-xl font-semibold text-zinc-900">{{ count($citiesSun['states']) }}</dd></div>
      @endunless
    </dl>
  </div>
</section>
...
<div class="container-portal pt-10 md:pt-12 pb-12 md:pb-16">
  ...
  @if($citiesSun['results'] !== null)
    <section class="mb-12 md:mb-16">
      <h2 class="text-2xl font-semibold text-zinc-900">{{ $citiesSun['results']->count() === \App\Themes\SunV2\CityIndexViewComposer::SEARCH_LIMIT ? __('portal.cities.results.heading_limit', ['anzahl' => number_format(\App\Themes\SunV2\CityIndexViewComposer::SEARCH_LIMIT, 0, ',', '.'), 'begriff' => $citiesSun['term']]) : trans_choice('portal.cities.results.heading', $citiesSun['results']->count(), ['anzahl' => number_format($citiesSun['results']->count(), 0, ',', '.'), 'begriff' => $citiesSun['term']]) }}</h2>
        ...
        <div class="mt-4 card p-6 md:p-10 max-w-3xl">
          <div class="flex flex-col sm:flex-row sm:items-start gap-5">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-zinc-700 leading-relaxed">{{ $citiesSun['land'] ? __('portal.cities.results.empty_land', ['begriff' => $citiesSun['term'], 'land' => $citiesSun['land']]) : __('portal.cities.results.empty', ['begriff' => $citiesSun['term']]) }}</p>
              <div class="mt-6"><a href="{{ $citiesSun['land'] ? route('portal.cities.index', ['land' => $citiesSun['landSlug']]) : route('portal.cities.index') }}" class="btn-secondary">{{ __('portal.cities.results.reset') }}</a></div>
            </div>
          </div>
        </div>
      @else
  ...
  @if($citiesSun['top']->isEmpty())
    <div class="card p-6 md:p-10 max-w-3xl">
      <div class="flex flex-col sm:flex-row sm:items-start gap-5">
        <span class="size-14 shrink-0 rounded-2xl bg-brand-50 text-brand flex items-center justify-center" aria-hidden="true"><x-sun.icon name="search-x" class="size-7" /></span>
        <div class="flex-1 min-w-0">
          <h2 class="text-2xl font-semibold text-zinc-900">{{ __('portal.cities.empty.heading') }}</h2>
          <p class="mt-2 text-zinc-700 leading-relaxed">{{ __('portal.cities.empty.text') }}</p>
  ...
  @else
    <section>
      <h2 class="text-2xl font-semibold text-zinc-900">{{ $citiesSun['land'] ? __('portal.cities.top.heading_land', ['land' => $citiesSun['land']]) : __('portal.cities.top.heading') }}</h2>
        ...
        @foreach($citiesSun['top'] as $city)
          <a href="{{ \App\Support\CityUrl::fromSlug($city['slug']) }}" class="card-interactive p-4 flex items-center gap-3 min-w-0">
            ...
            <span class="min-w-0">
              ...
              <span class="block text-sm text-zinc-500">{{ trans_choice('portal.cities.top.count', $city['count'], ['anzahl' => number_format($city['count'], 0, ',', '.')]) }}</span>
            </span>
          </a>
        @endforeach
      </div>
  ...
  @if($citiesSun['land'] && $citiesSun['groups']->isNotEmpty())
    <section class="mt-12 md:mt-16">
      <h2 class="text-2xl font-semibold text-zinc-900">{{ __('portal.cities.alphabet.heading', ['land' => $citiesSun['land']]) }}</h2>
      <nav aria-label="{{ __('portal.cities.alphabet.label') }}" class="mt-4 flex flex-wrap gap-2">
  ...
  @elseif(! $citiesSun['land'] && $citiesSun['states'] !== [])
    <section class="mt-12 md:mt-16">
      <h2 class="text-2xl font-semibold text-zinc-900">{{ __('portal.cities.states.heading') }}</h2>
      <ul class="mt-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-x-6 gap-y-1">
        @foreach($citiesSun['states'] as $state)
          <li><a href="{{ $state['url'] }}" class="flex justify-between items-center min-h-11 gap-4 hover:text-brand"><span class="font-medium text-zinc-900">{{ $state['label'] }}</span><span class="text-sm text-zinc-500">{{ trans_choice('portal.cities.states.places_count', $state['count'], ['anzahl' => number_format($state['count'], 0, ',', '.')]) }}</span></a></li>
        @endforeach
      </ul>
    </section>
  @endif
